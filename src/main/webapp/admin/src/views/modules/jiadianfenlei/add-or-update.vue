<template>
	<div class="addEdit-block" :style='{"minHeight":"calc(100vh - 200px)","padding":"20px 0 30px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
		...
		>
			<template >
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 24px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="input" v-if="type!='info'"  label="家电分类" prop="jiadianfenlei">
					<el-input v-model="ruleForm.jiadianfenlei" placeholder="家电分类" clearable  :readonly="ro.jiadianfenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 24px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' v-else class="input" label="家电分类" prop="jiadianfenlei">
					<el-input v-model="ruleForm.jiadianfenlei" placeholder="家电分类" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 24px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="upload" v-if="type!='info' && !ro.image" label="image" prop="image">
					<file-upload
						tip="点击上传image"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 24px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="upload" v-else-if="ruleForm.image" label="image" prop="image">
...
export default {
	data() {
		return {
			...
			rules: {
				jiadianfenlei: [
					{ required: true, message: '家电分类不能为空', trigger: 'blur' },
				],
				image: [
				],
			},
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `jiadianfenlei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jiadianfenleiCrossAddOrUpdateFlag = false;
											this.parent.search();
