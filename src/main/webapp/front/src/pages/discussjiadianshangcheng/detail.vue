...
<div>
	<div :style='{"padding":"20px","margin":"10px auto","borderRadius":"0","background":"rgba(255, 150, 27, 0.50)","display":"flex","width":"80%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/discussjiadianshangcheng?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"minHeight":"640px","padding":"0","flexWrap":"wrap","background":"#f5f5f5","display":"flex","width":"30%","position":"relative"}'>
			...
			<div class="info" :style='{"width":"100%","padding":"20px 10px","margin":"0","background":"#fff"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 auto 10px","color":"#fff","outline":"none","borderRadius":"10px","background":"linear-gradient(136deg, #F67055 0%, #FFC054 100%)","width":"100%","lineHeight":"40px","fontSize":"14px","height":"40px","order":"2"}' v-if="btnAuth('discussjiadianshangcheng','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #CCCCCC","cursor":"pointer","padding":"0 10px","margin":"0 auto 10px","color":"#CCCCCC","outline":"none","borderRadius":"10px","background":"none","width":"49%","lineHeight":"40px","fontSize":"14px","height":"40px","order":"6"}' v-if="btnAuth('discussjiadianshangcheng','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"1px solid #CCCCCC","cursor":"pointer","padding":"0 10px","margin":"0 auto 10px","color":"#CCCCCC","outline":"none","borderRadius":"10px","background":"none","width":"49%","lineHeight":"40px","fontSize":"14px","height":"40px","order":"6"}' v-if="btnAuth('discussjiadianshangcheng','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        tablename: 'discussjiadianshangcheng',
        baseUrl: '',
        breadcrumbItem: [
          {
            name: '家电商城评论表'
    ...
    methods: {
      ...
      sizeChange(size){
		...
		backClick(){
			...
			this.$router.push({path: '/index/discussjiadianshangcheng', query: params});
		},
		// 下载
		download(file){
			if(!file) {
      ...
      getDiscussList(page) {
        this.$http.get('discussdiscussjiadianshangcheng/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussdiscussjiadianshangcheng/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussdiscussjiadianshangcheng/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		  ...
		  this.$http.post('discussjiadianshangcheng/update',this.detail).then(res=>{
		...
		// 修改
		editClick(){
			this.$router.push(`/index/discussjiadianshangchengAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此家电商城评论表？')
			  .then(_ => {
			    this.$http.post('discussjiadianshangcheng/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
