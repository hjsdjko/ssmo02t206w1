...
<div :style='{"width":"100%","padding":"0 10%","margin":"0 auto","position":"relative","background":"none"}'>
    ...
    <div class="section-title" :style='{"border":"1px solid #D3D3D3","margin":"10px 0","color":"#FF961B","textAlign":"center","background":"#fff","fontSize":"28px","lineHeight":"54px","fontWeight":"bold"}'>我的订单</div>
                        ...
                        if(item.type==1||item.type==3) {
                            ...
                            this.$http.post(table+'/update', this.user).then(res => {
                                ...
                                this.$http.post(`orders/update`, item).then(res => {
                                    if(item.tablename == `jiadianshangcheng`){
                                        // 加回库存数量
                                        good.alllimittimes = good.alllimittimes + item.buynumber;
                                        this.$http.post(`jiadianshangcheng/update`, good).then(res => {
                                        });
                                    }
                                    this.$message({
